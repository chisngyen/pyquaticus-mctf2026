...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, help='Resume from checkpoint path (e.g., ./ray_test/iter_0/)', default=None)
    parser.add_argument('--start-iter', type=int, help='Starting iteration number if resuming', default=0)
    ...
    if args.gpu:
        ...
        ppo_config = ppo_config.framework("torch").resources(num_gpus=1)
    else:
        print("💻 CPU Training")
        ppo_config = ppo_config.framework("torch").resources(num_gpus=0)
    
    # Increase timeout to reduce warnings
    ppo_config = ppo_config.env_runners(sample_timeout_s=600.0)
    ...
    # Resume from checkpoint or build new
    if args.resume:
    ...
    for i in range(start_iter, 8001):
        print("Looping: ", i)
        start = time.time()
        algo.train()
        end = time.time()
